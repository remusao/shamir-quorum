...
/* global sjcl */
/* eslint-disable no-param-reassign, no-restricted-globals, no-bitwise */
...
export function padLeft(str, multipleOfBits) {
  let missing;
...
export function construct(bits, arr, radix, size) {
...
function testRandom(bits) {
  ...
  const elems = Math.ceil(bits / 32);
  const int = 123456789;
  const arr = new Uint32Array(elems);
...
function getRNG(type) {
  ...
  if (type && type === 'nodeCryptoRandomBytes') {
    config.typeCSPRNG = type;
    return nodeCryptoRandomBytes;
  }
  if (type && type === 'browserCryptoGetRandomValues') {
    config.typeCSPRNG = type;
    return browserCryptoGetRandomValues;
  }
  if (type && type === 'browserSJCLRandom') {
    runCSPRNGTest = false;
    config.typeCSPRNG = type;
...
function getShares(secret, numShares, threshold) {
  ...
  for (i = 1, len = numShares + 1; i < len; i += 1) {
    shares[i - 1] = {
      x: i,
      y: horner(i, coeffs),
    };
...
function constructPublicShareString(bits, id, data) {
  ...
  const idHex = padLeft(id.toString(config.radix), idPaddingLen);
...
export function init(bits, rngType) {
  ...
  if (
    ...
    throw new Error(
      `Number of bits must be an integer between ${defaults.minBits} and ${
        defaults.maxBits
      }, inclusive.`,
    );
  }
  ...
  for (i = 0; i < config.size; i++) {
    ...
    if (x >= config.size) {
      x ^= primitive; // Bitwise XOR assignment
      x &= config.maxShares; // Bitwise AND assignment
    }
  }
...
function seedRNG(data, estimatedEntropy, source) {
  let bytes;
  let rand;
  ...
  if (
    ...
    throw new Error(
      `Invalid share : Number of bits must be an integer between ${
        defaults.minBits
      } and ${defaults.maxBits}, inclusive.`,
    );
  }
...
export function share(secret, numShares, threshold, padLength) {
  let neededBits;
...
export function newShare(id, shares) {
  ...
  if (id && shares && shares[0]) {
    const shareComponents = extractShareComponents(shares[0]);
    return constructPublicShareString(shareComponents.bits, id, combine(shares, id));
  }
