...
Sha256.prototype.update = function (message) {
  ...
  let code; let index = 0; let i; const length = message.length; const
    blocks = this.blocks;
...
Sha256.prototype.finalize = function () {
  ...
  this.finalized = true;
  const blocks = this.blocks; const
...
Sha256.prototype.hash = function () {
  ...
  let h = this.h7; const blocks = this.blocks; let j; let s0; let s1; let maj; let t1; let t2; let ch; let ab; let da; let cd; let
    bc;
...
function HmacSha256(key, is224, sharedMemory) {
  let i; const
    type = typeof key;
  if (type === 'string') {
    const bytes = []; const length = key.length; let index = 0; let
      code;
    for (i = 0; i < length; ++i) {
